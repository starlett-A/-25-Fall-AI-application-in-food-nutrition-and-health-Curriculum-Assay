...
cat("  ✅ Random Forest feature importance plot saved\n")
...
if(nrow(lasso_coef_df) > 0) {
  lasso_plot_data <- lasso_coef_df
  lasso_plot_data$Metabolite <- factor(lasso_plot_data$Metabolite,
                                       levels = rev(lasso_plot_data$Metabolite))
  
  p_lasso <- ggplot(lasso_plot_data, aes(x = Coefficient, y = Metabolite, 
                                         fill = Coefficient > 0)) +
    geom_col(width = 0.7) +
    scale_fill_manual(values = c("red", "blue"), 
                      labels = c("Negative association", "Positive association"),
                      name = "Association with sarcopenia") +
    labs(x = "Lasso Regression Coefficient",
         y = NULL,
         title = paste("Metabolites Selected by Lasso Regression (n =", nrow(lasso_coef_df), ")"),
         subtitle = "Positive coefficient indicates up-regulation in Old group") +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          plot.subtitle = element_text(hjust = 0.5),
          axis.text.y = element_text(size = 9))
  
  ggsave("Lasso_Coefficients.png", p_lasso, width = 10, height = max(4, nrow(lasso_coef_df)*0.5), dpi = 600)
  cat("  ✅ Lasso coefficients plot saved\n")
}
...
if(length(high_confidence_metabolites) > 0) {
  ...
  cat("\n  ✅ Multi-method feature selection summary table saved\n")
}
...
if(length(high_confidence_metabolites) >= 3) {
  ...
  # 8.1 Creating heatmap for high confidence metabolites 
  if(length(high_confidence_metabolites) >= 3) {
    
    cat("  Creating heatmap for high confidence metabolites...\n")
    
    # Extracting top 10 metabolites
    top_n <- min(10, nrow(high_conf_df))
    top_metabolites <- head(high_conf_df[order(-high_conf_df$SelectedBy, 
                                               -high_conf_df$VIP_Score), ], top_n)$Metabolite
    
    # Making sure metabolites are in my data
    top_metabolites <- top_metabolites[top_metabolites %in% colnames(X)]
    
    if(length(top_metabolites) > 1) {
      # Preparing the data
      plot_data <- as.data.frame(X[, top_metabolites])
      plot_data$Sample <- rownames(X)
      plot_data$Group <- Y
      ...
      plot_data_long <- plot_data %>%
        ...
        mutate(Metabolite = factor(Metabolite, levels = top_metabolites))
      
      # Data normalizing by rows
      ...
      # Creating heatmap
      p <- ggplot(plot_data_scaled, aes(x = Sample, y = Metabolite, fill = Expression_scaled)) +
        geom_tile() +
        scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                             midpoint = 0, name = "Expression\n(z-score)") +
        facet_grid(~Group, scales = "free_x", space = "free_x") +
        labs(x = "Samples", y = "Metabolites",
             title = paste("Top", length(top_metabolites), "High-Confidence Metabolites Expression"),
             subtitle = "Rows scaled to z-score") +
        theme_bw() +
      ...
      ggsave("Top_HighConfidence_Metabolites_Heatmap_ggplot.png", p, 
             width = 10, height = 6, dpi = 600)
      cat("  ✅ ggplot heatmap is saved!\n")
    } 
    
    # Adding metabolites annotations
    # Creating another plot to display metabolites' properties
    if(length(top_metabolites) > 0) {  
      metab_info <- high_conf_df[match(top_metabolites, high_conf_df$Metabolite), 
                                 c("Metabolite", "SelectedBy", "VIP_Score")]
      metab_info$SelectedBy <- factor(metab_info$SelectedBy)
      
      p2 <- ggplot(metab_info, aes(x = Metabolite, y = VIP_Score, fill = SelectedBy)) +
        geom_bar(stat = "identity") +
        labs(x = "Metabolite", y = "VIP Score", 
             title = "Metabolite Properties",
             fill = "Selected by\n(methods)") +
        theme_bw() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1),
              plot.title = element_text(hjust = 0.5, face = "bold"))
      
      ggsave("Metabolite_Properties_Bar.png", p2, width = 10, height = 5, dpi = 600)
      cat("  ✅ Metabolite Properties Bar plot is saved!\n")
    }
    
  } # 闭合内层 if(length(high_confidence_metabolites) >= 3)
  ...
  # Making sure for enough metabolites
  if(nrow(high_conf_df) >= 5) {
    top5_metabolites <- head(high_conf_df$Metabolite, 5)
    top5_metabolites <- top5_metabolites[top5_metabolites %in% colnames(X)]
    
    if(length(top5_metabolites) > 0) {
      top5_expr <- X[, top5_metabolites, drop = FALSE]
      
      # Calculate MEAN values for each group
      for(i in 1:length(top5_metabolites)) {
        metab <- top5_metabolites[i]
      ...
      cat("    No top metabolites found in the data.\n")
    }
  } else {
    cat("    Less than 5 high-confidence metabolites available.\n")
  }
  
} else {  
  cat("  Less than 3 high-confidence metabolites available, skipping discriminative analysis.\n")
}
