...
if (any(is.na(group_vector))) {
  cat("⚠️  Warnings: Some sample names are not matched with neither 'Young' or 'Old' and will be grouped by its default order.\n")
...
plsda_result <- opls(x = data_matrix_filtered,   # Use preprocessed data
                     ...
                     crossvalI = 7,              # ropls's default number of cross-validation: 7-fold 
                     info.txtC = "none",         # no long and complex info output
                     fig.pdfC = "none")          # no auto-save PDF ver.

cat("  ✅  Successfully built PLS-DA model! \n")
...
cat(sprintf("  🎉 Core result: Found %d metabolites whose VIP > 1!\n", 
...
cat("  ✅ Done with saving PLS-DA results!\n")  
...
dev.off()
cat("  ✅ Samplse score plot saved as 'ropls_PLSDA_Score_Plot.png'\n")

# PDF ver.
pdf("Fig1_PLSDA_Score_Clean.pdf", width = 8, height = 6)
plot(plsda_result, typeVc = "x-score", parAsColFcVn = group_factor)
dev.off()
cat("✅ Saved as 'Fig1_PLSDA_Score_Clean.pdf'。\n")
...
cat("✅ Done with saving VIP bar plot\n")
...
cat("  ✅ Done with saving preprocessing analysis summary.\n")
